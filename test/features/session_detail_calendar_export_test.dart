...
import 'package:flo_compass/features/session_detail/widgets/session_detail_sticky_actions.dart';
...
void main() {
  ...
  testWidgets('desktop hero shows Add to calendar action', (tester) async {
    ...
    await tester.pumpWidget(
      MultiProvider(
        ...
        child: MaterialApp(
          localizationsDelegates: testLocalizationDelegates,
          supportedLocales: testSupportedLocales,
          home: const SessionDetailScreen(sessionId: 's-001'),
        ),
      ),
    );
    await tester.pump();
    ...
    expect(find.byType(SessionDetailDesktopActions), findsOneWidget);
    ...
    expect(find.text('Calendar file downloaded'), findsOneWidget);
  });

  testWidgets('narrow session detail shows calendar icon action', (
    tester,
  ) async {
  ...
  testWidgets('logistics tab shows Add to calendar label', (tester) async {
    ...
    expect(find.text('Export .ics'), findsNothing);
  });
}
